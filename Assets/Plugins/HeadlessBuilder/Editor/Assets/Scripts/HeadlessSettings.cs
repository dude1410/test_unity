...
public class HeadlessSettings : ScriptableObject {
	...
	public Il2CppCompilerConfiguration GetIl2CppCompilerConfiguration()
	{
		return Il2CppCompilerConfiguration.Release;
	}

	// TBR inject
	public ScriptingImplementation GetScriptingImplementation()
	{
	...
	public BuildTarget GetBuildTarget()
	{
		...
		if (valuePlatform == HeadlessEditor.WINDOWS && valueArchitecture == HeadlessEditor.X64)
		{
			buildTarget = BuildTarget.StandaloneWindows64;
		}
		if (valuePlatform == HeadlessEditor.WINDOWS && valueArchitecture == HeadlessEditor.X86)
		{
			buildTarget = BuildTarget.StandaloneWindows;
		}
		if (valuePlatform == HeadlessEditor.OSX && valueArchitecture == HeadlessEditor.X64)
		{
			buildTarget = HeadlessAPI.VersionedBuildTargetOSX();
		}
		if (valuePlatform == HeadlessEditor.LINUX && valueArchitecture == HeadlessEditor.X64)
		{
			buildTarget = BuildTarget.StandaloneLinux64;
		}
		if (valuePlatform == HeadlessEditor.LINUX && valueArchitecture == HeadlessEditor.X86)
		{
